...
passwords. If you did not modify the identity provider during installation,
to allow access, you must choose a different identity provider and
...
xref:../install_config/sssd_for_ldap_failover.adoc.adoc#setting-up-for-ldap-failover[configuring SSSD for LDAP failover].
====
...
provider for advanced authentication configurations, such as:
* xref:../install_config/sssd_for_ldap_failover.adoc.adoc#setting-up-for-ldap-failover[LDAP failover]
* link:https://github.com/openshift/request-header-saml-service-provider[SAML authentication]
